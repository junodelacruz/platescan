...
import { getFoodLog, getTodayEntries, deleteFoodEntry, getCalorieGoal } from '../services/storageService';
...
export default function HomeScreen({ navigation }) {
  ...
  const totalProtein = Math.round(todayEntries.reduce((s, e) => s + (e.items?.reduce((si, i) => si + (Number(i.protein) || 0), 0) ?? 0), 0));
  const totalCarbs = Math.round(todayEntries.reduce((s, e) => s + (e.items?.reduce((si, i) => si + (Number(i.carbs) || 0), 0) ?? 0), 0));
  const totalFat = Math.round(todayEntries.reduce((s, e) => s + (e.items?.reduce((si, i) => si + (Number(i.fat) || 0), 0) ?? 0), 0));
...
function EntryRow({ item, onDelete, navigation }) {
  ...
  // Defined inside EntryRow so isDark is accessible
  ...
  const styles = {
    entryRow: { paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: colors.border },
    entryTopSection: { width: '100%' },
    ...
    badgeText: { fontSize: 10, fontWeight: '700', color: colors.background, textTransform: 'uppercase', letterSpacing: 0.5 },
